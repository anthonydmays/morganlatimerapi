...
// This prevents the case where someone would want to debug a node_module that
// has husky as devDependency and run npm install from node_modules directory
function isInNodeModules(dir) {
  // INIT_CWD holds the full path you were in when you ran npm install
  // (supported also by yarn and pnpm) See https://docs.npmjs.com/cli/run-script
 ...
 * @param requireRunNodePath - path to run-node resolved by require e.g.
 *     /home/typicode/project/node_modules/.bin/run-node
 * @param isCI - true if running in CI
 */
function install(
    huskyDir, requireRunNodePath = require.resolve('.bin/run-node'), isCI) {
  console.log('husky > setting up git hooks');
  // First directory containing user's package.json
  const userPkgDir =
      pkg_dir_1.default.sync(path_1.default.join(huskyDir, '..'));
  ...
  // Get directory containing .git directory or in the case of Git submodules,
  // the .git file
