...
function handleArgs(cmd, args, opts) {
  ...
  opts = Object.assign({extendEnv: true, env: {}}, opts);
  ...
  if (opts.__winShell === true) {
    ...
    parsed =
        {command: cmd, args, options: opts, file: cmd, original: {cmd, args}};
  } else {
  ...
  return {cmd: parsed.command, args: parsed.args, opts, parsed};
}
...
function getStream(process, stream, {encoding, buffer, maxBuffer}) {
  ...
  if (!buffer) {
    // TODO: Use `ret = util.promisify(stream.finished)(process[stream]);` when
      ...
      Promise
          ...
          .then(arr => {
            ...
            if (result.error || result.code !== 0 || result.signal !== null) {
              const err = makeError(result, {joinedCmd, parsed, timedOut});
...
module.exports.sync = (cmd, args, opts) => {
  ...
  if (result.error || result.status !== 0 || result.signal !== null) {
    const err = makeError(result, {joinedCmd, parsed});
