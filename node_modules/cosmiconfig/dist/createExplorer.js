...
class Explorer {
  ...
  validateConfig() {
    ...
    config.searchPlaces.forEach(place => {
      ...
      if (!loader) {
        throw new Error(`No loader specified for ${
            getExtensionDescription(
                place)}, so searchPlaces item "${place}" is invalid`);
      }
    });
  }
  ...
  loadPackageProp(filepath, content) {
    ...
    const packagePropValue =
        getPropertyByPath(parsedContent, this.config.packageProp);
  ...
  loadFileContent(mode, filepath, content) {
    ...
    const loader = mode === MODE_SYNC ? this.getSyncLoaderForFile(filepath) :
  ...
  loadSync(filepath) {
    ...
    return cacheWrapper(this.loadSyncCache, absoluteFilePath, () => {
      ...
      const result =
          this.createCosmiconfigResultSync(absoluteFilePath, content);
      return this.config.transform(result);
    });
  }
}
