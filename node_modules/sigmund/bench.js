...
http.createServer(function(q, s) {
      sreq = q
      sres = s
      sres.end('ok') this.close(function() {
        setTimeout(function() {
          start()
        }, 200)
      })
...
function sparseFE2(obj, maxDepth) {
  ...
  function ch(v, depth) {
    if (depth > maxDepth)
      return if (
          typeof v === 'function' ||
          typeof v === 'undefined') return if (typeof v !== 'object' || !v) {
        soFar += v
        return
      }
    if (seen.indexOf(v) !== -1 || depth === maxDepth) return seen.push(v)
      soFar += '{'
      Object.keys(v).forEach(function(k, _, __) {
        // pseudo-private values.  skip those.
        if (k.charAt(0) === '_')
          return var to = typeof v[k] if (
                     to === 'function' || to === 'undefined') return soFar +=
              k + ':'
          ch(v[k], depth + 1)
      })
      soFar += '}'
  ...
  function ch(v, depth) {
    ...
    if (seen.indexOf(v) !== -1 || depth === maxDepth) return seen.push(v)
      ...
      Object.keys(v).forEach(function(k, _, __) {
        ...
        if (k.charAt(0) === '_')
          return var to = typeof v[k] if (
                     to === 'function' || to === 'undefined') return soFar += k
          ch(v[k], depth + 1)
      })
  }
  ch(obj, 0)
  ...
  function ch(v, depth) {
    ...
    if (seen.indexOf(v) !== -1 || depth === maxDepth) return seen.push(v)
      ...
      for (var k in v) {
        // pseudo-private values.  skip those.
        if (k.charAt(0) === '_')
          continue var to = typeof v[k] if (
              to === 'function' || to === 'undefined') continue soFar += k
          ch(v[k], depth + 1)
      }
  }
  ch(obj, 0)
...
exports.compare = {
  ...
  'JSON.stringify (guarded)': function() {
    var seen = [] return JSON.stringify(test, function(k, v) {
