var nopt = require('../'), test = require('tap').test,
...
test(
    'Empty String should not be true when type is single item Array',
    function(t) {
...
test(
    '~ path is resolved to ' + (isWin ? '%USERPROFILE%' : '$HOME'),
    function(t) {
      var path = require('path'), the

      if (isWin) {
        the = { key: 'USERPROFILE', dir: 'C:\\temp', val: '~\\val' }
      }
      else {
        the = { key: 'HOME', dir: '/tmp', val: '~/val' }
      }
...
test('Unknown options are not parsed as numbers', function(t) {
  var parsed = nopt(
      {'parse-me': Number}, null, ['--leave-as-is=1.20', '--parse-me=1.20'], 0)
...
test('Check types based on name of type', function(t) {
  var parsed =
      nopt({'parse-me': {name: 'Number'}}, null, ['--parse-me=1.20'], 0)
  t.equal(parsed['parse-me'], 1.2)
  t.end()
})
...
test('other tests', function(t) {
  ...
  ;
  [['-v', {version: true}, []], ['---v', {version: true}, []],
   [
     'ls -s --no-reg connect -d', {loglevel: 'info', registry: null},
     ['ls', 'connect']
   ],
   ...
   ['--no-color false', {color: true}, []], ['--no-color', {color: false}, []],
   ['--color false', {color: false}, []],
   ['--color --logfd 7', {logfd: 7, color: true}, []],
   ['--color=true', {color: true}, []], ['--logfd=10', {logfd: 10}, []],
   ...
   [
     '-t one -t null -t three four five null', {t: ['one', 'null', 'three']},
     ['four', 'five', 'null']
   ],
   ['-t foo', {t: ['foo']}, []], ['--no-t', {t: ['false']}, []],
   ['-no-no-t', {t: ['true']}, []],
   ['-aoa one -aoa null -aoa 100', {aoa: ['one', null, '100']}, []],
   ['-str 100', {str: '100'}, []], ['--color always', {color: 'always'}, []],
   ...
   ['--path', {'path': null}, []], ['--path .', {'path': process.cwd()}, []]]
      .forEach(function(test) {
        var argv = test[0].split(/\s+/), opts = test[1], rem = test[2],
            actual = nopt(test[3] || types, test[4] || shorthands, argv, 0),
            parsed = actual.argv delete actual.argv
        for (var i in opts) {
